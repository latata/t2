  ...
  return (<div className="row">
    <div className="col-md">
      <div>
        ...
        {get(student, 'phoneNo2') && <div className="d-flex align-items-center mb-2">
          <Icon name="phone" className="mr-2" /> {get(student, 'phoneNo2')}
                                     </div>}
        <div className="d-flex align-items-center mb-2">
          <Icon name="envelope-closed" className="mr-2" /> <a
    ...
    <div className="col-md">
      <h6>Grupy</h6>
      {get(student, 'groups', [])
        .map(group => (<div key={group._id}><Link to={`/group/${group._id}`}>{group.code}</Link>
        </div>))}
    </div>
          </div>);
}
...
function StudentPayments({ student }) {
  return (<table className="table">
    ...
    <tbody>
      {student && student.studentPayments.map(payment => (<tr key={payment._id}>
        <td>{payment.amount} PLN</td>
        <td>{payment.operationDate.substr(0, 10)}</td>
        <td><Link to={`/banktransfer/${payment.paymentBankTransfer}/assign`}><Icon
          name="link-intact"
        />
        </Link>
        </td>
    ...
    return (<div className="m-3">
      ...
      >{`${get(this, 'state.student.firstName')} ${get(this, 'state.student.lastName')}`}
        <small className="text-muted"><Link
          to={`/student/edit/${get(this, 'state.student._id')}`}
        ><Icon name="pencil" />
                                      </Link>
        </small>
      </h1>
